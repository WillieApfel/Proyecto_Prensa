...
function lable_edit_juegos($obj){
    global $connect;
    $sql= mysqli_query($connect,"SELECT c.home_club, c.visitante, el.nombrec AS 'local', ev.nombrec AS 'visit' FROM calendario_liga c 
    INNER JOIN equipos el ON c.home_club=el.id INNER JOIN equipos ev ON c.visitante=ev.id WHERE c.id='".$obj."'");
    $row=mysqli_fetch_array($sql);

    return array($row['visit'],$row['local'],$row['home_club'],$row['visitante']);
}
